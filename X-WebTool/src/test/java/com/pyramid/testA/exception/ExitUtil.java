 ...
 * 
 * Hadoop code that attempts to call {@link System#exit(int)} or
 * {@link Runtime#halt(int)} MUST invoke it via these methods.
 */
public final class ExitUtil {
    ...
    /**
     * An exception raised when a call to {@link #terminate(int)} was called and
     * system exits were blocked.
     */
    ...
    /**
     * Inner termination: either exit with the exception's exit code, or, if system
     * exits are disabled, rethrow the exception.
     * 
     * @param ee
     *            exit exception
     */
    ...
    /**
     * Forcibly terminates the currently running Java virtual machine. The exception
     * argument is rethrown if JVM halting is disabled.
     * 
     * @param ee
     *            the exception containing the status code, message and any stack
     *            trace.
    ...
    /**
     * Like {@link #terminate(int, String)} but uses the given throwable to build
     * the message to display or throw as an {@link ExitException}.
     * <p>
     * 
     * @param status
     *            exit code to use if the exception is not an ExitException.
     * @param t
     *            throwable which triggered the termination. If this exception is an
     *            {@link ExitException} its status overrides that passed in.
     ...
     *
     * @param status
     *            exit code to use if the exception is not a HaltException.
     * @param t
     *            throwable which triggered the termination. If this exception is a
     *            {@link HaltException} its status overrides that passed in.
     * @throws HaltException
     *             if {@link System#exit(int)} is disabled.
     */
    ...
    /**
     * Terminate the current process. Note that terminate is the *only* method that
     * should be used to terminate the daemon processes.
     *
     * @param status
     *            exit code
     * @param msg
    ...
    /**
     * Handler for out of memory events -no attempt is made here to cleanly shutdown
     * or support halt blocking; a robust printing of the event to stderr is all
     * that can be done.
     * 
     * @param oome
     *            out of memory event
